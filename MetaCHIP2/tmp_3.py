...
def boxplot_matrix_COG(args):
    ...
    column_order_list = ['J', 'A', 'K', 'L', 'B', 'D', 'Y', 'V', 'T', 'M', 'N', 'Z', 'W', 'U', 'O', 'X', 'C', 'G', 'E',
                         'F', 'H', 'I', 'P', 'Q', 'R', 'S']
    ...
    for detected_cate in column_order_list_detected:
        column_order_list_detected_with_description.append(
            '%s_%s' % (detected_cate, cate_to_description_dict[detected_cate]))
    ...
    if with_functional_description is True:
        category_num_df = pandas.DataFrame(category_num_arrary, index=genome_name_list,
                                           columns=column_order_list_detected_with_description)
    else:
        category_num_df = pandas.DataFrame(category_num_arrary, index=genome_name_list,
                                           columns=column_order_list_detected)
