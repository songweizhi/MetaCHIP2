...
option_list = list(
  make_option(c("-i", "--input"), type="character", help="input file name",  metavar="character"),
  make_option(c("-o", "--out"),   type="character", help="output plot name", metavar="character"));
...
label_text = paste("COG category (filled: ", row.names(my_data)[row_num-1], ", unfilled: ", row.names(my_data)[row_num], ")", sep="")

ggplot() + 
  geom_boxplot(data = stack(my_data[1:(row_num-2),]), aes(x = ind, y = values)) +

  geom_point(data = data.frame(x = factor(colnames(my_data)), y = as.numeric(my_data[row_num-1,])),
             aes(x=x, y=y), color=color_list_blast, shape=shape_list_blast, fill=color_list_blast, stroke=0.8, size=2.5) +
  
  geom_point(data = data.frame(x = factor(colnames(my_data)), y = as.numeric(my_data[row_num,])),
             aes(x=x, y=y), color = color_list_tree,  shape = shape_list_tree, stroke = 0.8, size = 2.5) +
  ...
  theme(panel.border = element_blank(), 
        ...
        axis.text.y = element_text(colour="black",size=9,angle=0,hjust=.5,vjust=.5,face="plain"),
  ...
  labs(x = label_text, y = 'Proportion')
